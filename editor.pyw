...
class Document:
    def __init__(self, Frame, TextWidget, FileDir=''):
        ...
        self.file_name = 'Code' if not FileDir else os.path.basename(FileDir)
...
class Editor:
    def __init__(self, master):
        ...
        filemenu.add_command(label="Compile", command=self.compile_file)
    ...
    def compile_file(self, *args):
        ...
        file_dir = "myProgram1.txt"
        file = open(file_dir, 'w')
        codeTxt = self.tabs[ curr_tab ].textbox.get(1.0, 'end')
        if not codeTxt:
            exit()
        file.write(codeTxt)
        file.close()
